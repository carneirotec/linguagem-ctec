...
#senão_se __arm__
defina_tipo caractere *va_list;
#defina _tcc_alignof(type) ((inteiro)&((estrutura {caractere c;type x;} *)0)->x)
#defina _tcc_align(addr,type) (((sem_sinal)addr + _tcc_alignof(type) - 1) \
                               & ~(_tcc_alignof(type) - 1))
#defina va_start(ap,last) ap = ((caractere *)&(last)) + ((tamanho_de(last)+3)&~3)
#defina va_arg(ap,type) (ap = (vazio *) ((_tcc_align(ap,type)+tamanho_de(type)+3) \
